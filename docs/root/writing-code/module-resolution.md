...
Eryx resolves `require()` paths through three layers: **VFS** (virtual filesystem bundles), **embedded libraries** (baked into the executable), and the **real filesystem**. All three can be active simultaneously - VFS and embedded are checked first, with the filesystem as the final fallback.
...
Native modules are shared libraries (`.dll` on Windows, `.so` on Linux, `.dylib` on macOS) that expose a C entrypoint. They silently shadow script libraries with the same base name - if both `foo.luau` and `foo.dll` exist, the native module is loaded.

### Filesystem Native Modules

When a `TYPE_FILE` library is resolved, eryx checks for a native library alongside it before reading the script:
...
In an `ERYX_EMBED` build, native modules are compiled directly into the executable. They appear in the `g_embedded_native_modules` table and are resolved via `@eryx` like any other embedded library. The `luau_module_info` export is suppressed (not needed since the entrypoint is registered directly in the table).

## Resolution Order

### Alias paths (`@alias/library`)
...
| -------------------------------- | ------------------------------------------------------------------------------ |
| Regular file `foo/bar.luau`      | `foo/` + library path                                                          |
| `init.luau` file `foo/init.luau` | `foo/` + library path (the directory the init file represents, not its parent) |
| VFS library                      | Try VFS first, then fall back to filesystem                                    |
| Embedded library                 | Try embedded first, then fall back to filesystem                               |

**3. `@eryx`** - Resolves to eryx's own standard libraries.

| Build mode     | Resolution                                                       |
| -------------- | ---------------------------------------------------------------- |
| Embedded build | Check embedded library table first                               |
...
Relative paths resolve from the calling file's directory. For `init.luau` files, `./` refers to the **parent** of the directory the init file sits in (i.e. the directory containing the folder that the init library represents), not the directory containing the init file itself.

The resolution chain for relative paths:

1. **VFS** - If the caller is a VFS library, try resolving within the VFS namespace first.
2. **Embedded** - If the caller is an embedded library, try resolving within the embedded library table.
...
* VFS libraries are loaded with chunk names prefixed `@@vfs/` (e.g. `@@vfs/src/app.luau`)
* Relative requires from VFS libraries resolve within the VFS namespace first
...
* Native module shadowing does **not** apply to VFS libraries (VFS only contains scripts)

### Embedded

Embedded libraries are eryx's own standard library, compiled directly into the executable. They are organized by key (e.g. `encoding/init`, `compression/zlib`). Embedded libraries can be either **scripts** (Luau source) or **native** (C++ entrypoints compiled into the binary). When the build uses `ERYX_EMBED`:

* Embedded libraries are loaded with chunk names prefixed `@@eryx/` (e.g. `@@eryx/encoding/init`)
* `@eryx` alias resolves to embedded libraries before falling back to the filesystem
* Relative requires from embedded libraries resolve within the embedded namespace first
* Native embedded modules are checked before script libraries for the same key
...
* **Standard libraries** are embedded
* User code can `require("@eryx/...")` to reach embedded libraries
* Embedded libraries can require each other via relative paths
...
A directory can act as a library by containing an `init.luau` (or `init.lua`) file. When you `require("./mydir")`, eryx finds `mydir/init.luau`.
